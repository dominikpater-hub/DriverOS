...
export class KnowledgeEngine implements IKnowledgePort {
  ...
  async searchKnowledge(query: KnowledgeSearchQuery): Promise<KnowledgeEntrySnapshot[]> {
    const entries = await this.storage.searchEntries({
      ...
      tags: query.tags ? { tags: query.tags } as any : undefined
    ...
    for (const entry of entries) {
      ...
      if (version && version.language === query.language) {
        snapshots.push(this.snapshotVersion(version));
      }
    }
  ...
  /**
   * Convert internal version to snapshot + trust level
   */
  private snapshotVersion(version: KnowledgeVersion): KnowledgeEntrySnapshot {
    const trustLevel = this.determineTrustLevel(version);
    ...
    return {
      ...
      country: version.entryId as any, // Would need full entry lookup
      domain: "TRAFFIC_LAW" // Would need full entry lookup
    };
  }
  ...
  private determineTrustLevel(version: KnowledgeVersion): TrustLevel {
    ...
    if (version.validUntil && isDateInPast(version.validUntil)) {
      return TrustLevel.VERIFIED_STALE;
    }

...
export class WorkflowEngine implements IWorkflowPort {
    ...
    try {
      ...
      switch (stepDef.kind) {
        case StepKind.SHOW_KNOWLEDGE:
          result = await this.executeShowKnowledge(stepDef, instance);
          break;
        ...
        default:
          throw new Error(`Unknown step kind: ${stepDef.kind}`);
      }
    } catch (error) {
      // Offline fallback if available
      if (stepDef.fallback) {
        instance.currentStepId = stepDef.fallback;
        result = { stepId: stepDef.fallback, kind: stepDef.kind };
      } else {
        throw error;
      }
    }
  ...
  private async executeShowKnowledge(
    ...
    // Get knowledge from Knowledge Engine

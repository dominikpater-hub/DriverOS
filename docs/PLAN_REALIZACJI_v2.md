...
- [x] **2.4 EmergencyCard → Knowledge (D-02)** — karta ratunkowa dostaje **wersjonowaną historię dowodową**: treść karty jest publikowana jako niemutowalny `KnowledgeVersion` na wspólnej osi wersji, `EmergencyCardSnapshot` niesie `versionId`+`trustLevel`, a `executeEmergencyCard` zapisuje do Incidentu REALNĄ wersję (rozwiązywalną przez `getKnowledgeVersion()` do audytu) zamiast `card.id as any`. Pole `currentVersionId` opcjonalne → brak migracji Postgresa. **Decyzja architektoniczna:** pełne scalenie do jednego agregatu (`EmergencyCard` = `KnowledgeEntry`) świadomie odroczone — dotyka prawnie-krytycznej warstwy storage (InMemory+Postgres+publisher) w 5 plikach; transza dowodowa daje właściwą wartość (audytowalność) przy niskim ryzyku. +1 test.
- [x] **2.5 pełna analiza nakładania reguł** — walidacja wykrywa teraz **semantyczne nakładanie różnych warunków** (nie tylko identyczne zbiory): dowodzi nakładania przez **skonstruowany i zweryfikowany świadek-input** (matcher realny → zero fałszywych alarmów). Nakładanie + różny outcome → twardy błąd; nakładanie + ten sam outcome → benign tie (warn). Zakresy liczbowe rozłączne (gt 100 vs lt 50) nie są flagowane; nakładające (gt 100 vs gt 200) tak. +4 testy.

**M2 KOMPLETNE.** 2.1–2.5 zrobione.

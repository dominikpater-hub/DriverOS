...
export enum TrustLevel {
  ...
  /** Verified knowledge but past review date */
  T2_VERIFIED_STALE = "T2_VERIFIED_STALE",
  /** AI-assisted answer, explicit disclaimer */
  T3_AI_ASSISTED = "T3_AI_ASSISTED",
  /** Offline fallback: emergency contacts, universal rights */
...
export enum StepKind {
  ...
  /** Wait for user or external event */
  WAIT = "WAIT"
}
...
export enum Capability {
  ...
  /** @deprecated alias for OFFLINE_STORAGE — kept for pre-platform engine code */
  STORAGE = "STORAGE"
}
...
// SemVer is a branded string (canonical, Faza 1) — serializable, package-friendly.
...
export const asSemVer = (value: string): SemVer => {
  if (!isSemVer(value)) throw new Error(`Invalid SemVer: "${value}"`);
  return value as SemVer;
};
/** Parse into comparable components (the wire form stays a branded string). */
export const parseSemVer = (v: SemVer): { major: number; minor: number; patch: number } => {
  const [major, minor, patch] = String(v).split(".").map((n) => parseInt(n, 10));
  return { major, minor, patch };
};
export const semVerToString = (v: SemVer): string => String(v);
